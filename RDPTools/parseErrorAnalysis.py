...
def removeFailedChopseq(chop_seq_dict, seq_dict):
	for seqID in list(seq_dict.keys()):
...
def get_error_count(seq_dict):
	...
	for key in sorted(count_dict.keys()):
		print("%s\t%s\t%s\t%s" %(key, count_dict[key], float(100*count_dict[key])/float( len(list(seq_dict.keys())) ), float(100*key*count_dict[key])/float(total_mismatches+total_indels )))	
...
def get_error_count_by_std(seq_dict):
	...
	for std_ID in list(seq_error_dict.keys()):
...
def qscore_histogram(seq_dict):
	...
	for key in sorted(count_dict.keys()):
		...
		print("%s\t%s\t%s" %(key, count_dict[key], float(100*count_dict[key])/float( len(list(seq_dict.keys())) )))	
...
def qscore_seqpassed(seq_dict):
	...
	while ( q <= max_qscore):
		if q not in list(qscore_dict.keys()):
			q +=1
			continue;
		error_dict = qscore_dict.get(q)
		val = str(q)
		for e in range(0, max_error+1):
			if e in list(error_dict.keys()):
...
def	calSeqError(seq_dict, chop_seq_dict):
	...
	for seqID in list(chop_seq_dict.keys()):
		if (seqID not in list(seq_dict.keys()) ):
			continue
		if (seqID.startswith("#=") ):	
			continue
...
if __name__ == "__main__":
	...
	if options.start_model_pos:
		if not options.end_model_pos:
			options.end_model_pos = sys.maxsize 
	if options.end_model_pos:
		if not options.start_model_pos:
			options.start_model_pos = -sys.maxsize -1
